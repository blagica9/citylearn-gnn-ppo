...
class GNNLSTMAgent:
    ...
    def update(self, episodes=100):
        ...
        with open("training_log.csv", mode="a", newline="") as log_file:
            ...
            for ep in range(10, 10 + episodes):
                obs = self.env.reset()
                obs_buffer, log_probs, values, rewards, actions_taken = [], [], [], [], []
                ...
                while not done:
                    ...
                    actions_taken.append(actions_tensor.detach())
                ...
                loss.backward()
                self.optimizer.step()
                if (ep + 1) % 10 == 0 or (ep + 1) == 10 + episodes:
                    torch.save(self.policy.state_dict(), f"checkpoint_ep{ep+1}.pt")
        ...
        plt.title("Training Progress (Episodes 11-110)")
        plt.xlabel("Episode")
        plt.ylabel("Total Reward")
        plt.grid()
        plt.savefig("training_curve_extended.png")
        print("\u2705 Training complete. Extended curve saved as 'training_curve_extended.png'.")
